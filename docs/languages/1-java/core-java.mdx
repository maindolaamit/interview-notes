...
`==` is used to compare the reference of the object, if two variables holds reference to the same object while `equals` is used to compare the content of the object.
...
<details>
  <summary>String Buffer vs String Builder</summary>
  ```java
  public class Main {
    public static void main(String[] args) {
      ...
      System.out.println(stringBuilder); // Outputs: Hello World!

      // StringBuffer
      StringBuffer stringBuffer = new StringBuffer();
      stringBuffer.append("Hello ");
      stringBuffer.append("World!");
      System.out.println(stringBuffer); // Outputs: Hello World!
  }
}
  ```
  Both StringBuilder and StringBuffer allow you to create a string in parts, but the difference lies in their synchronization mechanism. If your application is single-threaded or you do not need to share these objects across threads then using StringBuilder can be faster than StringBuffer as it does not have any overhead of synchronization.
</details>
...
<Tabs>
  ...
  <TabItem value="StringBuilder">
    - `StringBuilder` is synchronized while `StringBuffer` is not synchronized.
    - `StringBuilder` is high performant for modifying String.
  </TabItem>
  <TabItem value="StringBuffer">
    - `StringBuffer` is synchronized i.e. thread-safe. Multiple threads can share the object without any problem.
...
<details>
<summary>Inner Static classes</summary>

  When to use static inner classes

  - Helper methods : Use static inner classes when you need helper methods that are closely related to another class, but don't require access to its instance variables.
  - Utility classes : When creating utility classes with methods that operate on a specific type or data structure, consider using a static inner class to encapsulate the utility functions.
  - Private implementation details : If you have private implementation details that are only needed within a single method or group of related methods, a static inner class can be a good choice for hiding those details.
...
public class Calculator {
    public static void main(String[] args) {
        ...
        double mean = MathHelper.calculateMean(new int[] {1, 2, 3, 4, 5});
...
<details>
  <summary>
    Each JVM (Java Virtual Machine) runs in its own isolated environment, with its own class loader and memory space.
    This isolation ensures that singleton instances are unique within their respective JVMs.
  </summary>
  - Class Loading: Each JVM has its own class loader, responsible for loading classes and creating their instances. When
  a singleton class is loaded for the first time within a JVM, its single instance is created and maintained thereafter.
  - Memory Isolation: JVMs have separate memory spaces, preventing interference between classes and objects in different
  JVMs. This means singleton instances in one JVM are distinct from those in other JVMs.
  - Distributed Systems: In distributed systems where multiple JVMs interact, each JVM will have its own singleton
  instances, potentially leading to multiple instances of the same singleton class across the system.
  - Serialization: When a singleton is serialized and deserialized, its unique instance identity might not be preserved,
  potentially leading to multiple instances within a JVM. Careful handling of serialization is crucial to maintain the
  singleton pattern.
</details>
...
`Serialization` is a process in Java where we convert an object into a byte stream which can be saved in a file or sent over the network. We can use `ObjectInputStream` and `ObjectOutputStream` to serialize and deserialize the object.
...
public class SerializationExample {
    public static void main(String[] args) throws Exception {
        ...
        SerializationExample s1 = (SerializationExample) objectInputStream.readObject();
...
- We can use Factory pattern to create an object of the class based on the input parameter.

```java
public class FactoryExample {
    public static void main(String[] args) {
        Factory factory = new Factory();
        factory.getObject("A").method();
        factory.getObject("B").method();
    }
}
