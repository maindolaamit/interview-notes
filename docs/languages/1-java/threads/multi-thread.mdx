...
<Tabs>
<TabItem value="Thread Class[Old]">
...
<TabItem value="Runnable Interface">
...
// Preferred way to create threads in Java
...
To control concurrent modifications, the Java programming language defines the 
`synchronized` keyword for two purposes:
- It can restrict access to a shared Object (to preserve invariants),
    so only a Thread can execute a routine at any given time.
- It propagates changes from the current Thread local memory to the global memory,
    that’s available to all running threads of executions.
...
<Tabs>
    <TabItem value="ThreadLocal">

    - ThreadLocal is a class in Java that allows you to store data that will be accessible only by a specific thread. It is a thread-local variable.
    - Each thread that accesses the variable (via its get or set method) has its own, independently initialized copy of the variable.
    ...
    <TabItem value="ThreadLocalRandom">
...
**Use Case:**
- When you need to return a value from a thread.
- When you need to throw a checked exception from a thread.
...
## Synchronization Components Overview

### What are Atomic and Volatile variables?
Both variables are used in concurrent programming to avoid data inconsistency issues caused by concurrent access to the same variable.
<Tabs>
<TabItem value="Atomic">
    - Atomic variables are thread-safe and can be used without synchronization. Atomic variables are available in the java.util.concurrent.atomic package. Atomic variables are used to perform atomic operations on single variables. Volatile variables are used to indicate that a variable's value will be modified by different threads.
    - Ensures changes made to the variable is visible to other threads.
    - Useful for counters, accumulators, and flags that are accessed by multiple threads.
    ```java
...
<TabItem value="Volatile">
    - Volatile variables are used to perform atomic operations on single variables. Archives this by preventing compiler and and CPU from caching.
    - Lightweight & faster than synchronization mechanism, since it does not involve locking.
    - Useful for simple flags or counters that are accessed by multiple threads.
    - Does not guaranteee atomicity for compound oprations like incrementing a counter.
...
### Modern Approaches (Commonly Used)
1. **CompletableFuture**
   - Most recommended for async operations in modern Java
   - Best for: Chain of async operations, combining multiple async tasks
   - Perfect for microservices, API calls, and non-blocking operations

2. **Atomic Variables**
   - Preferred choice for thread-safe counters and flags
   - Best for: Simple shared state management without locks
   - Highly performant for single variable operations

### Situational Use Cases
<details>
<summary>
   **CountDownLatch**
   - Still relevant but specific use case
   - Best for: One-time synchronization where one or more threads wait for a set of operations
   - Common in startup sequences or initialization patterns
</summary>
    ```java
    public class Main {
        public static void main(String[] args) {
...
<summary>
   **Semaphore**
   - Used in specific resource management scenarios
   - Best for: Controlling access to limited resources (connection pools, thread pools)
   - Common in resource pool implementations
</summary>
    ```java
    public class Main {
        public static void main(String[] args) {
...
### Legacy Approaches (Less Common in Modern Applications)
<details>
<summary>
   **CyclicBarrier**
   - Less common in modern applications
   - Replaced by more flexible alternatives like CompletableFuture
   - Consider only for specific algorithmic problems where multiple threads need synchronized starts
</summary>
    CycleBarrier is a class in Java that allows a set of threads to wait until all threads reach a common barrier point.
    It is a synchronization aid that allows a set of threads to wait until all threads reach a common barrier point.
...
<summary>
   **Exchanger**
   ...
   - Very specific use case for two-thread data exchange
   - Modern alternatives like queues or reactive streams are preferred
</summary>
    ```java
    public class Main {
        public static void main(String[] args) {
...
- Use Atomic variables for simple shared state
- Consider higher-level abstractions (e.g., ExecutorService) over direct thread management
- For complex scenarios, consider reactive programming frameworks
:::

## Advanced Examples
For more complex multi-thread scenarios, see the [Advanced Multi‑Threading page](/docs/languages/java/threads/multi-thread-advanced).
