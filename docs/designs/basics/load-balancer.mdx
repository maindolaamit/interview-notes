...
There is a variety of load balancers available in the market, some of them are:
- NGINX
- HAProxy
- Apache


![./img/load-balancer.png](./img/load-balancer.png)

<details>
    <summary>**Classifications of Load Balancers**</summary>
...
<details>
    <summary>**Load balancer vs reverse proxy**</summary>
...
:::danger
To prevent the load balancer from becoming a single point of failure, organizations often deploy redundant load balancers in a high-availability configuration.

In this setup, multiple load balancers work together, with one acting as primary and others as backups. They continuously monitor each other's status, and if the primary load balancer experiences issues, a backup automatically assumes control to maintain uninterrupted service.
:::

### Load balacing algorithms

Load balacing algorithms/techniques are:
- **Round Robin** : Directs traffic to the server in a round robin manner.
- **Weighted Round Robin** : Directs traffic to the server based on the weight of the server.
- **Least Connections** : Directs traffic to the server with the least number of connections.
- **Least Response Time** : Directs traffic to the server with the least response time.
- **IP Hash** : Directs traffic to the server based on a hash of the request IP address.
- **Consistent Hashing** : Directs traffic to the server based on a hash of the request.
