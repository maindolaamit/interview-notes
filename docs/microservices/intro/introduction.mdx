...
While both microservice architecture and service-oriented architecture (SOA) structure a system as a set of services, they exhibit significant differences in their approach to inter-service communication, data management, service size, and technology stacks.
...
The differences between SOA and microservice architecture are summarized in the following table from source:

| Feature              | SOA                                                                       | Microservices                                                     |
| ------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| Inter-service Comm. | Smart pipes (ESB), heavyweight protocols (SOAP, WS*)                     | Dumb pipes (message broker), lightweight protocols (REST, gRPC) |
| Data                | Global data model, shared databases                                       | Data model and database per service                             |
| Typical Service Size  | Larger, often integrating monolithic applications                            | Smaller, more focused                                          |

**Microservices as a Form of Modularity**

The sources highlight that microservices can be viewed as a form of modularity, promoting loose coupling and independent development and deployment. Each microservice has a clearly defined API, which acts as an "impermeable boundary" that encapsulates its internal implementation. This encapsulation enhances modularity by preventing developers from bypassing the API and accessing internal components directly.

**Key Considerations**

While microservices offer potential advantages over SOA for modern application development, choosing between the two architectures depends on the specific needs and constraints of a project. 
...
**In conclusion, the microservice architecture represents an evolution of service-based architectures, emphasizing agility, scalability, and loose coupling through fine-grained services, lightweight communication protocols, and independent data management.** 
