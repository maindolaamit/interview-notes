...
It requires `version number` to be of three parts:
- MINOR : It is incremented when new, backwards compatible functionality is introduced to the public API. 
- PATCH : It is incremented when backwards compatible bug fixes are introduced.
- MAJOR : It is incremented when backwards incompatible changes are introduced to the public API.
...
- **RPC**: It is a protocol. It is a request–response protocol for message-passing. It allows a program to cause a subroutine or procedure to execute in another address space (commonly on another computer on a shared network) without the programmer explicitly coding the details for this remote interaction.
...
<details>
<summary>What important HTTP status codes and error messages ?</summary>
...
1. **Schema**: A Schema in GraphQL is a collection of types. It defines the types and fields that can be queried.
...
### If your REST endpoint only supports **JSON output** and the client requests **XML format**, what error code your API should return :
<details>
    <summary>
        Answer
    </summary>
...
- **Use this if the client requests XML via the `Accept` header** and your API does not support XML.
- **Example Response:**
```http
...
- **Use this if the client is sending data in XML format (`Content-Type: application/xml`)** but your API only accepts JSON.
- **Example Response:**
```http
...

{
  ...
  "message": "This API only supports application/json input."
}
  ```

#### **Which One to Use?**
- **If the client is sending a request and expects XML (`Accept: application/xml`) → Use `406 Not Acceptable`.**
- **If the client is sending XML in the request body (`Content-Type: application/xml`) → Use `415 Unsupported Media Type`.**
</details>
